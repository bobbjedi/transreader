...
async function fetchSection(section) {
  ...
  $('#content p').each(async (i, el) => {
    ...
    dictionary.push({ en: word, ru: translation });
  });
}
let counter = 0;
async function main(sections) {
  for (const section of sections) {
    counter++;
    console.log('Fetching section:', section, 'counter:', counter, 'total:', sections.length);
    try {
      await fetchSection(section);
    } catch (e) {
      console.error('Error fetching', section, e);
    }
    await new Promise(r => setTimeout(r, 1500)); // пауза между запросами
    fs.writeFileSync('dictionary.json', JSON.stringify(dictionary, null, 2));
  }

  console.log('Done. Total words:', dictionary.length, 'sections:', counter);
